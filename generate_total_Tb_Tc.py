# this file is built for generating a training model trained by the whole Tb and Tc dataset
...
from utils.piplines import train_epoch_frag, evaluate_frag, PreFetch_extra
...
dataset_list = ['Tc_JCIM_normalized','Tb_JCIM_normalized']
...
for i in range(len(dataset_list)):
    ...
    fragmentation = JT_SubGraph(scheme='MG_original_reference')
    ...
    raw_loader = DataLoader(dataset, collate_fn=collate_fraggraphs, batch_size=len(dataset), shuffle=False, num_workers=0, worker_init_fn=np.random.seed(1000))

    fetched_data = PreFetch_extra(raw_loader)
    ...
    t0 = time.time()
    per_epoch_time = []
    early_stopping = EarlyStopping(patience=params['earlystopping_patience'], path=checkpoint_path +'/checkpoint_total_' + params['Dataset'] + 'FraGAT' + '.pt')
    ...
    with tqdm(range(params['max_epoch'])) as t:
        ...
        for epoch in t:
            ...
            epoch_val_loss, epoch_val_metrics = evaluate_frag(model, scaling, 
                                                                fetched_data.all_iter, 
                                                                fetched_data.all_batched_origin_graph_list,
                                                                fetched_data.all_batched_frag_graph_list, 
                                                                fetched_data.all_batched_motif_graph_list,
    ...
    _, epoch_raw_metrics = evaluate_frag(model, scaling, 
                                        fetched_data.all_iter, 
                                        fetched_data.all_batched_origin_graph_list,
                                        fetched_data.all_batched_frag_graph_list, 
                                        fetched_data.all_batched_motif_graph_list,
    ...
    path = params['Dataset'] + '/all'
    name = '{}_{}_{}'.format('Ensemble_0', params['Dataset'], 'AGCNet')
    results = pd.Series({'all_R2': epoch_raw_metrics.R2,
                        'all_MAE': epoch_raw_metrics.MAE,
                        'all_RMSE': epoch_raw_metrics.RMSE, 
                        'all_SSE': epoch_raw_metrics.SSE,
