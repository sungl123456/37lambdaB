# this file is built for find the suitable random seed in VLE prediction
...
from networks.AGC_VLE_multi import AGCNetVLEMultiTask
...
for j in range(len(dataset_list)):
    ...
    for i in range(20):
        ...
        model = AGCNetVLEMultiTask(net_params).to(device='cpu')
        ...
        t0 = time.time()
        per_epoch_time = []
        early_stopping = EarlyStopping(patience=params['earlystopping_patience'], path=checkpoint_path + '/checkpoint_ensemble_' + params['Dataset'] + '_AGCNet_VLE_MultiTask' + '.pt')
        ...
        with tqdm(range(params['max_epoch'])) as t:
            ...
            for epoch in t:
                ...
                t.set_postfix({'time': time.time() - start, 'lr': optimizer.param_groups[0]['lr'],
                    ...
                    'train_R2': epoch_val_metrics_T.R2, 'val_R2': epoch_val_metrics_Y.R2})
        ...
        _, epoch_raw_metrics_T, epoch_raw_metrics_Y = evaluate_frag_VLE(model,scaling_T,scaling_y1,fetched_data.all_iter, 
                                                                            fetched_data.all_batched_origin_graph_list_comp1, fetched_data.all_batched_origin_graph_list_comp2,
                                                                            fetched_data.all_batched_frag_graph_list_comp1, fetched_data.all_batched_frag_graph_list_comp2,
                                                                            fetched_data.all_batched_motif_graph_list_comp1, fetched_data.all_batched_motif_graph_list_comp2,
        ...
        wr.loc[i] = [seed, epoch_train_metrics_T.R2, epoch_val_metrics_T.R2,
                epoch_test_metrics_T.R2, epoch_raw_metrics_T.R2,
                epoch_train_metrics_Y.R2, epoch_val_metrics_Y.R2,
                epoch_test_metrics_Y.R2, epoch_raw_metrics_Y.R2,
                epoch_train_metrics_T.MAE, epoch_val_metrics_T.MAE, epoch_test_metrics_T.MAE,  epoch_raw_metrics_T.MAE,
                epoch_train_metrics_T.RMSE, epoch_val_metrics_T.RMSE, epoch_test_metrics_T.RMSE,  epoch_raw_metrics_T.RMSE,
                epoch_train_metrics_Y.MAE, epoch_val_metrics_Y.MAE, epoch_test_metrics_Y.MAE,  epoch_raw_metrics_Y.MAE,
                epoch_train_metrics_Y.RMSE, epoch_val_metrics_Y.RMSE, epoch_test_metrics_Y.RMSE,  epoch_raw_metrics_Y.RMSE]
