# this file was built for examine the AGC model could be used for Tb or Tc predictions
...
for z in range(len(dataset_list)):
    params['Dataset'] = dataset_list[z]
    ...
    if not os.path.exists(file_path):
        os.mkdir(file_path)
    save_file_path = os.path.join(file_path, '{}_{}_{}'.format(params['Dataset'], 'AGCNet_MG', time.strftime('%Y-%m-%d-%H-%M')) + '.csv')
    wr = pd.DataFrame(columns=['seed', 'train_R2', 'val_R2', 'test_R2', 'all_R2', 'train_MAE', 'val_MAE', 'test_MAE', 'all_MAE',
    ...
    for i in range(10):
        ...
        t0 = time.time()
        per_epoch_time = []
        early_stopping = EarlyStopping(patience=params['earlystopping_patience'], path=checkpoint_path +'/checkpoint_seed_' + params['Dataset'] + 'FraGAT' + '.pt')
        ...
        wr.loc[i] = [seed, epoch_train_metrics.R2, epoch_val_metrics.R2,
                    epoch_test_metrics.R2, epoch_raw_metrics.R2,
                    epoch_train_metrics.MAE, epoch_val_metrics.MAE,
                    epoch_test_metrics.MAE,  epoch_raw_metrics.MAE,
                    epoch_train_metrics.RMSE,  epoch_val_metrics.RMSE,
                    epoch_test_metrics.RMSE,  epoch_raw_metrics.RMSE]
