...
for i in range(len(dataset_list)):
    ...
    for j in range(10):
        ...
        _, epoch_val_metrics = evaluate_frag(model, scaling, fetched_data.val_iter, fetched_data.val_batched_origin_graph_list,
                                            fetched_data.val_batched_frag_graph_list, fetched_data.val_batched_motif_graph_list,
                                            fetched_data.val_targets_list, fetched_data.val_smiles_list, n_param)
        _, epoch_test_metrics, predict_test, true_test, smiles_test = evaluate_frag(model, scaling, fetched_data.test_iter, fetched_data.test_batched_origin_graph_list,
                                            fetched_data.test_batched_frag_graph_list, fetched_data.test_batched_motif_graph_list,
                                            fetched_data.test_targets_list, fetched_data.test_smiles_list, n_param, flag = True)
        ...
        test_results = pd.DataFrame({'SMILES':smiles_test, 'predicted value':predict_test, 'true value':true_test})
        file_path = '{}_{}_{}_{}'.format('Ensemble_0', 'test_results', params['Dataset'], 'AGCNet')
