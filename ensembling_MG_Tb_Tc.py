...
for i in range(len(dataset_list)):
    ...
    for j in range(1):
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=params['lr_reduce_factor'], patience=params['lr_schedule_patience'], verbose=False)
        ...
        with tqdm(range(params['max_epoch'])) as t:
            ...
            for epoch in t:
                ...
                model, epoch_train_loss, epoch_train_metrics = train_epoch_frag(model, optimizer, scaling,
                                                                                    fetched_data.all_iter, fetched_data.all_batched_origin_graph_list,
        ...
        path = '/Ensembles/'+ params['Dataset'] + '/MG_normalized'
