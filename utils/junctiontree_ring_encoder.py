...
class frag():
    def __init__(self, frag_doc, ring_doc):
        path = os.path.join('.//dataset', frag_doc + '.csv')
        data_from = os.path.realpath(path)
        df = pd.read_csv(data_from)
        pattern1 = np.array([df['First-Order Group'], df['SMARTs'], df['Priority']])

        path2 = os.path.join('.//dataset', ring_doc + '.csv')
        df2 = pd.read_csv(path2)
        pattern2 = np.array([df2['Ring Structure'], df2['SMARTs'], df2['Priority']])

        pattern = np.concatenate((pattern1,pattern2), axis=1)
        self.sorted_pattern = pattern[:, np.argsort(pattern[2, :])]
        self.frag_name_list = list(dict.fromkeys(self.sorted_pattern[0, :]))
        self.frag_dim = len(self.frag_name_list)#按照priority进行排序所有的name
