...
def train_epoch_frag_VLE3(model, optimizer, scaling, iter, 
                               batched_origin_graph_comp1, batched_origin_graph_comp2, 
                               batched_frag_graph_comp1, batched_frag_graph_comp2, 
                               batched_motif_graph_comp1, batched_motif_graph_comp2,
                               batched_Tb_comp1, batched_Tc_comp1, batched_Tb_comp2, batched_Tc_comp2,
                               batched_x1, batched_T, batched_Y, n_param = None):
    st = time.time()
    model.train()
    score_list = []
    target_T_list = []
    ...
    for i in iter:
        ...
        target_T = batched_T[i]
        target_Y = batched_Y[i]
        target_list.append([target_T,target_Y])
        target = torch.tensor([target_T,target_Y]).to(device = 'cpu')
        loss = model.loss(score,target)
        loss.backward()
        optimizer.step()
        score_list.append(score)
        # target_T_list.append(target_T)
...
def evaluate_frag_VLE3(model, scaling, iter, 
                               batched_origin_graph_comp1, batched_origin_graph_comp2, 
                               batched_frag_graph_comp1, batched_frag_graph_comp2, 
                               batched_motif_graph_comp1, batched_motif_graph_comp2,
                               batched_Tb_comp1, batched_Tc_comp1, batched_Tb_comp2, batched_Tc_comp2,
    ...
    for i in iter:
        ...
        score = model.forward(batch_origin_graph_comp1, batch_origin_graph_comp2,
                                  ...
                                  batch_Tb_comp2, batch_Tc_comp2,
                                  batch_x1)
        target_T = batched_T[i].float().to(device = 'cpu')
        target_Y = batched_Y[i].float().to(device = 'cpu')
        target_list.append([target_T,target_Y])
        target = batched_T[i].float().to(device='cpu')
        loss = model.loss(score, target)
        score_list.append(score)
        # target_list.append(target)
        epoch_loss += loss.detach().item()
