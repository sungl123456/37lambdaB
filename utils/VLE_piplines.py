...
def evaluate_frag_clf(model, iter, batched_origin_graph_comp1, batched_origin_graph_comp2, 
                               batched_frag_graph_comp1, batched_frag_graph_comp2, 
                               batched_motif_graph_comp1, batched_motif_graph_comp2,
                               batched_Tb_comp1, batched_Tc_comp1, batched_Tb_comp2, batched_Tc_comp2,
                               targets, smiles_comp1 = None, smiles_comp2 = None, flag=False):
    model.eval()
    score_list = []
    target_list = []
    epoch_loss = 0
    ...
    if score_list:
        ...
        # 使用阈值0.5进行二分类
        predictions = (all_scores >= 0.5).int()
        
        if predictions.dim() > 1:
            predictions = predictions.squeeze()
        if all_targets.dim() > 1:
            all_targets = all_targets.squeeze().int()
        
        epoch_eval_metrics = Metrics_classification(all_targets, predictions)
    if not flag:
        return epoch_loss, epoch_eval_metrics
    else:
        return epoch_loss, epoch_eval_metrics,all_scores, all_targets, predictions, smiles_comp1, smiles_comp2
...
def evaluate_frag_clf_attention_value(model, iter, 
    ...
    return all_scores, all_targets, predictions, attention_comp1_list, attention_comp2_list, smiles_list_comp1, smiles_list_comp2



def train_epoch_frag_VLE(model, optimizer, scaling_T, scaling_Y, iter, 
                               batched_origin_graph_comp1, batched_origin_graph_comp2, 
                               batched_frag_graph_comp1, batched_frag_graph_comp2, 
                               batched_motif_graph_comp1, batched_motif_graph_comp2,
                               batched_Tb_comp1, batched_Tc_comp1, batched_Tb_comp2, batched_Tc_comp2,
                               batched_x1, targets_T,targets_Y, n_param = None):
    st = time.time()
    model.train()
    score_list_T = []
    target_list_T = []
    ...
    for i in iter:
        ...
        score_T,score_Y = model.forward(batch_origin_graph_comp1, batch_origin_graph_comp2,
                                  batch_origin_node_comp1, batch_origin_node_comp2,
                                  batch_origin_edge_comp1, batch_origin_edge_comp2,
                                  batch_frag_graph_comp1, batch_frag_graph_comp2,
                                  batch_frag_node_comp1, batch_frag_node_comp2,
                                  ...
                                  batch_Tb_comp2, batch_Tc_comp2,
                                  batch_x1)
        target_T = targets_T[i].float().to(device='cpu')
        target_Y = targets_Y[i].float().to(device='cpu')
        loss = model.loss(score_T, target_T, score_Y, target_Y)
        loss.backward()
        optimizer.step()
        score_list_T.append(score_T)
        target_list_T.append(target_T)
        score_list_Y.append(score_Y)
        target_list_Y.append(target_Y)
        epoch_loss += loss.detach().item()
    score_list_T = torch.cat(score_list_T, dim = 0)
    target_list_T = torch.cat(target_list_T, dim = 0)
    epoch_train_metrics_T = Metrics(scaling_T.ReScaler(target_list_T.detach().to(device = 'cpu')), 
                                scaling_T.ReScaler(score_list_T.detach().to(device = 'cpu')),n_param)
    score_list_Y = torch.cat(score_list_Y, dim = 0)
    target_list_Y = torch.cat(target_list_Y, dim = 0)
    epoch_train_metrics_Y = Metrics(scaling_Y.ReScaler(target_list_Y.detach().to(device = 'cpu')), 
                                scaling_Y.ReScaler(score_list_Y.detach().to(device = 'cpu')),n_param)
    

    return model, epoch_loss, epoch_train_metrics_T, epoch_train_metrics_Y

def evaluate_frag_VLE(model, scaling_T, scaling_Y, iter, 
                               batched_origin_graph_comp1, batched_origin_graph_comp2, 
                               batched_frag_graph_comp1, batched_frag_graph_comp2, 
                               batched_motif_graph_comp1, batched_motif_graph_comp2,
                               batched_Tb_comp1, batched_Tc_comp1, batched_Tb_comp2, batched_Tc_comp2,
                               batched_x1, targets_T, targets_Y, smiles_comp1, smiles_comp2, n_param = None, flag = False):
    model.eval()
    score_list_T = []
    target_list_T = []
    score_list_Y = []
    ...
    score_list_T = torch.cat(score_list_T, dim = 0)
    target_list_T = torch.cat(target_list_T, dim = 0)
    epoch_eval_metrics_T = Metrics(scaling_T.ReScaler(target_list_T.detach().to(device = 'cpu')), 
                                scaling_T.ReScaler(score_list_T.detach().to(device = 'cpu')),n_param)
    score_list_Y = torch.cat(score_list_Y, dim = 0)
    target_list_Y = torch.cat(target_list_Y, dim = 0)
    epoch_eval_metrics_Y = Metrics(scaling_Y.ReScaler(target_list_Y.detach().to(device = 'cpu')), 
                                scaling_Y.ReScaler(score_list_Y.detach().to(device = 'cpu')),n_param)
    
    predict_T = scaling_T.ReScaler(score_list_T.detach().to(device='cpu'))
    true_T = scaling_T.ReScaler(target_list_T.detach().to(device='cpu'))
    predict_Y = scaling_Y.ReScaler(score_list_Y.detach().to(device='cpu'))
    true_Y = scaling_Y.ReScaler(target_list_Y.detach().to(device='cpu'))
    if flag:
        return epoch_loss, epoch_eval_metrics_T,epoch_eval_metrics_Y, predict_T, true_T, predict_Y, true_Y, smiles_comp1, smiles_comp2
    else:
        return epoch_loss, epoch_eval_metrics_T,epoch_eval_metrics_Y


def evaluate_frag_VLE_attention_value(model, scaling_T, scaling_Y, iter, 
                               batched_origin_graph_comp1, batched_origin_graph_comp2, 
                               batched_frag_graph_comp1, batched_frag_graph_comp2, 
                               batched_motif_graph_comp1, batched_motif_graph_comp2,
                               batched_Tb_comp1, batched_Tc_comp1, batched_Tb_comp2, batched_Tc_comp2,
                               batched_x1, targets_T, targets_Y, smiles_comp1, smiles_comp2, n_param = None):
    model.eval()
    score_list_T = []
    target_list_T = []
    score_list_Y = []
    ...
    for i in iter:
        ...
        score_T,score_Y, attention_comp1, attention_comp2 = model.forward(batch_origin_graph_comp1, batch_origin_graph_comp2,
                                  batch_origin_node_comp1, batch_origin_node_comp2,
                                  batch_origin_edge_comp1, batch_origin_edge_comp2,
                                  batch_frag_graph_comp1, batch_frag_graph_comp2,
                                  batch_frag_node_comp1, batch_frag_node_comp2,
        ...
        target_T = targets_T[i].float().to(device='cpu')
        target_Y = targets_Y[i].float().to(device='cpu')
        loss = model.loss(score_T,score_Y,target_T,target_Y)
        score_list_T.append(score_T)
        target_list_T.append(target_T)
        score_list_Y.append(score_Y)
        target_list_Y.append(target_Y)
    ...
    return epoch_loss, epoch_eval_metrics_T,epoch_eval_metrics_Y, predict_T, true_T, predict_Y, true_Y, attention_comp1_list, attention_comp2_list, smiles_comp1, smiles_comp2
