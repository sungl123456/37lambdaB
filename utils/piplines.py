...
class PreFetch(object):
    def __init__(self, train_loader, val_loader, test_loader, raw_loader, frag):
        if frag == 2:
            ...
            #     _batched_origin_graph, _batched_frag_graph, _batched_motif_graph, _batched_channel_graph, _targets, _smiles = batch
...
def train_epoch(model, optimizer, scaling, iter, batched_origin_graph, targets, smiles, n_param=None):
    ...
    epoch_loss = 0
    for i in iter:
        batch_origin_node = batched_origin_graph[i].ndata['feat'].to(device='cpu')
        batch_origin_edge = batched_origin_graph[i].edata['feat'].to(device='cpu')
        batch_origin_global = batched_origin_graph[i].ndata['global_feat'].to(device='cpu')
        batch_origin_graph = batched_origin_graph[i].to(device='cpu')
...
def train_epoch_frag(model, optimizer, scaling, iter, batched_origin_graph, batched_frag_graph, batched_motif_graph, targets, smiles, n_param=None):
    st = time.time()
    model.train()
    score_list = []
    target_list = []
    ...
    for i in iter:
        ...
        batch_frag_node = batched_frag_graph[i].ndata['feat'].to(device='cpu')
        batch_frag_edge = batched_frag_graph[i].edata['feat'].to(device='cpu')
        batch_frag_graph = batched_frag_graph[i].to(device='cpu')

        batch_motif_node = batched_motif_graph[i].ndata['feat'].to(device='cpu')
        batch_motif_edge = batched_motif_graph[i].edata['feat'].to(device='cpu')
        batch_motif_graph = batched_motif_graph[i].to(device='cpu')
...
def train_epoch_gcgat(model, optimizer, scaling, iter, batched_origin_graph, batched_frag_graph, batched_motif_graph, batched_channel_graph, batched_channel_index, targets, smiles, n_param=None):
    st = time.time()
    model.train()
    score_list = []
    target_list = []
    ...
    for i in iter:
        ...
        batch_channel_graph = batched_channel_graph[i].to(device='cpu')
...
def evaluate_frag(model, scaling, iter, batched_origin_graph, batched_frag_graph, batched_motif_graph, targets, smiles, n_param=None, flag=False):
    model.eval()
    score_list = []
    target_list = []
    epoch_loss = 0
...
def evaluate_gcgat(model, scaling, iter, batched_origin_graph, batched_frag_graph, batched_motif_graph, batched_channel_graph, batched_channel_index, targets, smiles, n_param=None, flag=False):
    model.eval()
    score_list = []
    target_list = []
    epoch_loss = 0
...
def evaluate_frag_descriptors(model, scaling, iter, batched_origin_graph, batched_frag_graph, batched_motif_graph, targets, smiles, n_param=None):
    model.eval()
    score_list = []
    target_list = []
    for i in iter:
...
def evaluate_gcgat_descriptors(model, scaling, iter, batched_origin_graph, batched_frag_graph, batched_motif_graph, batched_channel_graph, batched_channel_index, targets, smiles, n_param=None):
    model.eval()
    score_list = []
    target_list = []
    for i in iter:
