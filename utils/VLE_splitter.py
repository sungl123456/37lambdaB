...
class VLE_Splitter3(data.Dataset):
    def __init__(self, dataset):
        super().__init__()
        self.dataset = dataset
        
    def Random_Splitter(self, comp, seed = 100, frac_train = 0.8):
        self.length = len(comp)
        self.comp = comp
        mixtures = []              
        for i in range(self.length):
        ...
        for i in range(num_train,self.index):
            for j in range(self.length):
                if mixtures[indices[i]] == [comp[j][0],comp[j][1]] or[mixtures[indices[i]][1],mixtures[indices[i]][0]] == [comp[j][0],comp[j][1]]:
                    test_index.append(j)
        ...
        return  train_dataset, test_dataset, self.dataset