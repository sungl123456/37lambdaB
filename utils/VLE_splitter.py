...
class pair_Splitter3(data.Dataset):
    ...
    def Random_Splitter(self, comp, seed = 100, frac_train = 0.8, frac_val = 0.9):
        ...
        val_index = indices[num_train:num_val]
        test_index = indices[num_val:self.index]
...
class VLE_Splitter2(data.Dataset):
    ...
    def Random_Splitter(self, comp, seed = 100, frac_train = 0.8, frac_val = 0.9):
        ...
        for i in range(len(train_index)):
            for j in range(51):
                train_indices.append(train_index[i]*51+j)

        for i in range(len(val_index)):
            for j in range(51):
                val_indices.append(val_index[i]*51+j)
        
        for i in range(len(test_index)):
            for j in range(51):
                test_indices.append(test_index[i]*51+j)
