...
class AGCNetVLE(nn.Module):
    def __init__(self, net_params):
        ...
        self.linear_predict1 = nn.Sequential(
            ...
            nn.Linear(2 * net_params['hidden_dim'] + 4, net_params['hidden_dim'], bias=True),
    ...
    def forward(self, origin_graph_comp1, origin_graph_comp2, 
        ...
        concat_features = torch.cat([super_new_graph_comp1, super_new_graph_comp2], dim = -1)
        concat_features = torch.cat([concat_features,Tb_comp1, Tc_comp1, Tb_comp2, Tc_comp2], dim = -1)
        descriptors = self.linear_predict1(concat_features)
        output= torch.sigmoid(self.linear_predict2(descriptors))
        if get_attention:
