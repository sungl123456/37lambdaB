...
class AGCNetVLE(nn.Module):
    def __init__(self, net_params):
        ...
        self.linear_predict1_3 = nn.Sequential(
            nn.Linear(5, net_params['hidden_dim'], bias=True),
            nn.BatchNorm1d(net_params['hidden_dim']),
            nn.SiLU(),
        )
        self.shared_feature_extractor = nn.Sequential(
            nn.Linear(3*net_params['hidden_dim'], 2*net_params['hidden_dim'], bias=True),
            nn.SiLU(),
        ...
        nn.Linear(net_params['hidden_dim'], net_params['hidden_dim']//2, bias=True),
        nn.SiLU(),
        nn.Dropout(net_params['dropout']),
        nn.Linear(net_params['hidden_dim']//2, net_params['hidden_dim']//4, bias=True),
        nn.SiLU(),
        nn.Linear(net_params['hidden_dim']//4, 1, bias=True))
    ...
    def reset_parameters(self):
        ...
        for junction_layer in self.junction_heads_comp2:
            junction_layer.reset_parameters()
        for layer in [self.linear_predict1_1, self.linear_predict1_2, self.linear_predict1_3,
                     self.shared_feature_extractor, self.temperature_head, self.composition_head]:
    ...
    def forward(self, origin_graph_comp1, origin_graph_comp2, 
        ...
        # 物理特征处理
        concat_Tb_Tc = torch.cat([Tb_comp1, Tc_comp1, Tb_comp2, Tc_comp2, x1], dim=-1)
        ...
        shared_features = self.shared_feature_extractor(concat_features)
        
        # 多任务预测
        temperature_pred = self.temperature_head(shared_features)  # 泡点温度
        composition_pred = self.composition_head(shared_features)  # 气相组成
        ...
        if get_attention:
            ...
            return temperature_pred, composition_pred, attention_list_array_comp1, attention_list_array_comp2
        
        if get_descriptors:
            return temperature_pred, composition_pred, super_new_graph_comp1, super_new_graph_comp2
        else:
            return temperature_pred, composition_pred
    
    def loss(self, temperature_pred, temperature_target, composition_pred, composition_target):
        abs_error_y = torch.abs(composition_target - composition_pred)
        mean_abs_error_y = torch.mean(abs_error_y)

        relative_error_T = torch.abs(temperature_target - temperature_pred)
        mean_relative_error_T = torch.mean(relative_error_T)

        loss = torch.sqrt(mean_abs_error_y) + torch.sqrt(mean_relative_error_T)
