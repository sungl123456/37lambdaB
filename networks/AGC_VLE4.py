...
class AGCNetVLE4(nn.Module):
    ...
    def forward(self, origin_graph_comp1, origin_graph_comp2, 
        ...
        var = self.var_layer(descriptors)
        if get_attention:
        ...
        if get_descriptors:
            return output, var, super_new_graph_comp1, super_new_graph_comp2
        else:
            return output, var
    
    def loss(self, scores, targets, var):
        var = torch.exp(var)
        loss = torch.mean(0.5 * torch.log(var) + 0.5 * ((targets - scores) ** 2 / var))
        return loss