...
fpr_test, tpr_test, _ = roc_curve(test_target.detach().numpy().flatten().tolist(), test_score_list.detach().numpy().flatten().tolist())
fpr_all, tpr_all, _ = roc_curve(all_target.detach().numpy().flatten().tolist(), all_score_list.detach().numpy().flatten().tolist())
auc_score_test = auc(fpr_test, tpr_test)
auc_score_all = auc(fpr_all, tpr_all)
plt.plot(fpr_all, tpr_all, color='aqua', lw=2, label = 'ROC curve of total dataset')
plt.plot(fpr_test, tpr_test, color='darkorange', lw=2, label = 'ROC curve of total dataset')
plt.plot([0, 1], [0, 1], 'k--', lw=2)
plt.xlim([-0.050, 1.0])
plt.ylim([0.0, 1.05])
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('ROC Curve Comparison for Different Feature Sets (Micro-average)')
...
df_fpr_tpr_test = pd.DataFrame({'fpr_test':fpr_test, 'tpr_test':tpr_test})
df_auc_score_test  = pd.Series({'auc_score': auc_score_test})
...
df_fpr_tpr_all = pd.DataFrame({'fpr_all':fpr_all, 'tpr_all':tpr_all})
df_auc_score_all = pd.Series({'auc_score':auc_score_all}) 
...
# save_tpr_test = os.path.join('./library/' + path, '{}_{}_{}_{}_{}'.format('test_tpr_fpr',name, dataset_list[0], splitting_seed, time.strftime('%Y-%m-%d-%H-%M')) + '.csv')
# save_tpr_all = os.path.join('./library/' + path, '{}_{}_{}_{}_{}'.format('all_tpr_fpr',name, dataset_list[0], splitting_seed, time.strftime('%Y-%m-%d-%H-%M')) + '.csv')
df_test_results.to_csv(save_test_file_path,index= False)
df_all_results.to_csv(save_file_path, index=False)
df_fpr_tpr_test.to_csv(save_test_file_path,mode = 'a', header = False, index = False)
df_fpr_tpr_all.to_csv(save_file_path, mode='a', header = False, index = False)
df_auc_score_test.to_csv(save_test_file_path,mode = 'a', header = False, index = False)
df_auc_score_all.to_csv(save_file_path, mode='a', header = False, index = False)
