...
class Azeotrope_Dataset(object):
    ...
    def _prepare(self, params, smiles_2_graph, atom_featurizer, bond_featurizer, mol_featurizer, load, log_every, error_log):
        ...
        if os.path.exists(cache_comp1_path) and os.path.exists(cache_comp2_path) and load:
            ...
            save_graphs(cache_comp1_path, self.origin_graphs_comp1, labels={'values': self.values, 'valid_idx': valid_idx})
            save_graphs(cache_comp2_path, self.origin_graphs_comp2, labels={'values': self.values, 'valid_idx': valid_idx})
    ...
    def _prepare_frag(self, params, fragmentation, load, log_every, error_log):
        ...
        _frag_cache_file_path_comp2 = os.path.join(self.cache_file_path, params['Dataset'] +  '_comp2_' + self.name + '_frag')
        _motif_cache_file_path_comp1 = os.path.join(self.cache_file_path, params['Dataset'] +  '_comp1_' + self.name + '_motif')
        _motif_cache_file_path_comp2 = os.path.join(self.cache_file_path, params['Dataset'] +  '_comp2_' + self.name + '_motif')
        ...
        if os.path.exists(_frag_cache_file_path_comp2) and os.path.exists(_motif_cache_file_path_comp2) and load:
            print('Loading saved fragments and graphs for comp2 ...')
    ...
    def __getitem__(self,index):
        if self.whe_frag:
            return self.origin_graphs_comp1[index], self.origin_graphs_comp2[index], self.batched_frag_graphs_comp1[index], self.batched_frag_graphs_comp2[index], self.motif_graphs_comp1[index], self.motif_graphs_comp2[index], self.channel_graphs_comp1[index], self.channel_graphs_comp2[index],self.values[index], self.smiles1[index],self.smiles2[index]
        else:
            return self.origin_graphs_comp1[index], self.origin_graphs_comp2[index], self.values[index], self.smiles1[index],self.smiles2[index]
