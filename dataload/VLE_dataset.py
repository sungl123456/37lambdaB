...
class VLE_Dataset(object):
    ...
    def _prepare(self, params, smiles_2_graph, atom_featurizer, bond_featurizer, mol_featurizer, load, log_every, error_log):
        ...
        :param
        '''
        cache_comp1_path = os.path.join(self.cache_file_path, params['Dataset'] + '_comp1_' + self.name + '_VLE')
        cache_comp2_path = os.path.join(self.cache_file_path, params['Dataset'] + '_comp2_' + self.name +'_VLE')
        ...
        if os.path.exists(cache_comp1_path) and os.path.exists(cache_comp2_path) and load:
            ...
            self.x1 = label_dict_comp1['x1']
            self.T = label_dict_comp1['T']
            self.y1 = label_dict_comp1['y1']
            ...
            _label_x1 = self.df['x1']
            _label_T = self.df['T']
            _label_y1 = self.df['y1']
            ...
            self.x1 = F.zerocopy_from_numpy(np.nan_to_num(_label_x1).astype(np.float32))[self.valid_idx]
            self.T = F.zerocopy_from_numpy(np.nan_to_num(_label_T).astype(np.float32))[self.valid_idx]
            self.y1 = F.zerocopy_from_numpy(np.nan_to_num(_label_y1).astype(np.float32))[self.valid_idx]
            ...
            save_graphs(cache_comp1_path, self.origin_graphs_comp1, labels={'valid_idx': valid_idx, 'Tb_comp1':self.Tb_comp1, 'Tc_comp1':self.Tc_comp1, 'x1': self.x1, 'T': self.T, 'y1': self.y1})
            save_graphs(cache_comp2_path, self.origin_graphs_comp2, labels={'valid_idx': valid_idx, 'Tb_comp2':self.Tb_comp2, 'Tc_comp2':self.Tc_comp2})
    ...
    def _prepare_frag(self, params, fragmentation, load, log_every, error_log):
        ...
        _frag_cache_file_path_comp1 = os.path.join(self.cache_file_path, params['Dataset'] +  '_comp1_' + self.name + 'VLE_frag')
        _frag_cache_file_path_comp2 = os.path.join(self.cache_file_path, params['Dataset'] +  '_comp2_' + self.name + 'VLE_frag')
        _motif_cache_file_path_comp1 = os.path.join(self.cache_file_path, params['Dataset'] +  '_comp1_' + self.name + 'VLE_motif')
        _motif_cache_file_path_comp2 = os.path.join(self.cache_file_path, params['Dataset'] +  '_comp2_' + self.name + 'VLE_motif')
        ...
        if os.path.exists(_frag_cache_file_path_comp1) and os.path.exists(_motif_cache_file_path_comp1) and load:
            ...
            valid_idx = torch.tensor(self.valid_idx)
            save_graphs(_frag_cache_file_path_comp1, unbatched_frag_graphs_comp1, labels={'valid_idx': valid_idx, 'frag_graph_idx': frag_graph_idx_comp1})
            save_graphs(_motif_cache_file_path_comp1, self.motif_graphs_comp1, labels={'valid_idx': valid_idx})
        ...
        if os.path.exists(_frag_cache_file_path_comp2) and os.path.exists(_motif_cache_file_path_comp2) and load:
            ...
            valid_idx = torch.tensor(self.valid_idx)
            save_graphs(_frag_cache_file_path_comp2, unbatched_frag_graphs_comp2, labels={ 'valid_idx': valid_idx, 'frag_graph_idx': frag_graph_idx_comp2})
            save_graphs(_motif_cache_file_path_comp2, self.motif_graphs_comp2, labels={'valid_idx': valid_idx})
    ...
    def __getitem__(self,index):
        if self.whe_frag:
            return self.origin_graphs_comp1[index], self.origin_graphs_comp2[index], self.batched_frag_graphs_comp1[index], self.batched_frag_graphs_comp2[index], self.motif_graphs_comp1[index], self.motif_graphs_comp2[index], self.channel_graphs_comp1[index], self.channel_graphs_comp2[index], self.smiles1[index],self.smiles2[index], self.Tb_comp1[index], self.Tc_comp1[index], self.Tb_comp2[index], self.Tc_comp2[index], self.x1[index], self.T[index], self.y1[index]
        else:
            return self.origin_graphs_comp1[index], self.origin_graphs_comp2[index], self.smiles1[index], self.smiles2[index], self.Tb_comp1[index], self.Tc_comp1[index], self.Tb_comp2[index], self.Tc_comp2[index], self.x1[index], self.T[index], self.y1[index]
        
    def _prepare_channel(self):
        self.channel_graphs_comp1 = []
        self.channel_graphs_comp2 = []
        for _ in range(len(self.df['comp1'])):
            self.channel_graphs_comp1.append(create_channels())
        for _ in range(len(self.df['comp1'])):
